...
1. **ダニエルズ理論（VDOT）：**
ジャック・ダニエルズ博士による走力指標。現在の走力から**E（Easy）、M（Marathon）、T（Threshold）、I（Interval）、R（Repetition）**の5つの最適ペースを算出する。練習の「目的」と「強度」を1秒単位で一致させるための絶対的な基準。

2. **ピラミッド型モデル（Pyramidal Training）：**
マラソンランナーに最も適した強度配分。低強度(E)を底辺(70-80%)とし、中強度(M/T)を中段(15-20%)、高強度(I/R)を頂点(5-10%)とする。
